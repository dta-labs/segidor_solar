 ...
 *                                                                           *
 *                     SEGUIDOR SOLAR RCT v1.02 Capulín                      *
 *                                                                           *
...
int Toperacion = Tpmax * 1000 / 150;              // Tiempo máximo del pistón en milisegundos entre la cantidad de grados (10h)
int direccion               = 0;
int Anno                    = 2021;
int Mes                     = 1;
int Dia                     = 1;
...
void calcularTiempoOperacion() {
  verano = (Mes >= 4 && Mes <= 9) ? 19 : 18;      // Verano 7pm -> Abril - Septiembre
  grados = (verano - horaInicio) * 60 / 4;        // Grados por hora 1h => 15°
  int Tp = Tpmax * 1000;                          // Convertir el tiempo máximo del pistón a milisegundos
  Toperacion = Tp / grados;                       // Tiempo del pistón entre los grados a moverse
}
...
void calculaPosicion() {
  Serial.println("Calcular posición...");
  if (horaInicio <= Hora && Hora < verano) {
    ...
    posicionDeseada = (posicionDeseada <= grados) ? posicionDeseada : grados;
    ...
    //      posicionActual++;
      ...
      visualizarCadenasLCD("DTA-Labs Energia", "Posicion: ", angulo);
    }
  } else if (verano <= Hora && posicionActual != 0) {
    moveToLimit(-1);
    moveToLimit(-1);
    posicionActual = 0;
  }
...
void moveToLimit(int dir){
  ...
  if (modoDigitalAnalogico == 0) {
    ...
    do {
      ...
      while (ret == digitalRead(pintSensorHall) && temporizador < 1250) { // 1250
        delay(100);
        temporizador++;
        Serial.print(".");
      }
      if (temporizador < 1250 && temporizador > 0) {
  //        temporizador = 0;
